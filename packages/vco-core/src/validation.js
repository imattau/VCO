import { ED25519_SIGNATURE_LENGTH, FLAG_ZKP_AUTH, HEADER_HASH_LENGTH, MAX_VCO_SIZE, MULTICODEC_ED25519_PUB, PROTOCOL_VERSION, RESERVED_FLAG_MASK, } from "./constants.js";
...
function assertLength(fieldName, value, expected) {
    if (value.length !== expected) {
        throw new EnvelopeValidationError(`${fieldName} must be ${expected} bytes, received ${value.length}.`);
    }
}
function isZeroed(value) {
    return value.every((byte) => byte === 0);
}
function validateZkpExtension(envelope) {
    ...
    if (extension.inputsLength !== extension.publicInputs.length) {
        throw new EnvelopeValidationError("zkpExtension.inputsLength must match zkpExtension.publicInputs.length.");
    }
...
export function validateEnvelope(envelope) {
    if (envelope.header.version !== PROTOCOL_VERSION) {
        throw new EnvelopeValidationError(`Unsupported version ${envelope.header.version}; expected ${PROTOCOL_VERSION}.`);
    }
    if (envelope.payload.length > MAX_VCO_SIZE) {
        throw new EnvelopeValidationError(`Payload exceeds MAX_VCO_SIZE (${MAX_VCO_SIZE}).`);
    }
    assertLength("headerHash", envelope.headerHash, HEADER_HASH_LENGTH);
    assertValidPayloadMultihash(envelope.header.payloadHash);
    if (!Number.isInteger(envelope.header.flags) ||
    ...
    if (!Number.isInteger(envelope.header.nonce) ||
    ...
    if (isZkpAuth) {
        if (envelope.header.creatorId.length > 0 && !isZeroed(envelope.header.creatorId)) {
            throw new EnvelopeValidationError("creatorId must be empty or zeroed when FLAG_ZKP_AUTH is set.");
        }
        if (envelope.header.signature.length > 0 && !isZeroed(envelope.header.signature)) {
            throw new EnvelopeValidationError("signature must be empty or zeroed when FLAG_ZKP_AUTH is set.");
        }
        ...
        if (creatorKey.codec === MULTICODEC_ED25519_PUB &&
            envelope.header.signature.length !== ED25519_SIGNATURE_LENGTH) {
            throw new EnvelopeValidationError(`Ed25519 signatures must be ${ED25519_SIGNATURE_LENGTH} bytes.`);
        }
    }
