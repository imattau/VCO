...
export function getKnownPayloadTypes(): readonly number[] {
...
export function isKnownPayloadType(payloadType: number): boolean {
  return KNOWN_PAYLOAD_MULTICODECS.has(payloadType);
}

export function isSupportedPayloadType(payloadType: number): boolean {
  return SUPPORTED_PAYLOAD_MULTICODECS.has(payloadType);
}

export function assertValidPayloadType(payloadType: number): void {
  if (
  ...
  if (!isKnownPayloadType(payloadType)) {
    throw new EnvelopeValidationError(
      `Unknown payloadType ${payloadType}. It must be a registered multicodec identifier.`,
    );
  }
}

export function assertSupportedPayloadType(payloadType: number): void {
  if (!isSupportedPayloadType(payloadType)) {
    throw new EnvelopeValidationError(
      `Unsupported payloadType ${payloadType}. Supported runtime codecs: ${getSupportedPayloadTypes().join(", ")}.`,
    );
  }
}
