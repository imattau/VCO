...
import { ShieldCheck, ShieldX, Zap, X, AlertCircle } from "lucide-react";
...
function hex(arr: Uint8Array | undefined | null) {
  if (!arr) return "none";
...
export function VcoInspector({ message, onClose }: Props) {
  ...
  try {
    ...
    return (
      <div className="w-80 bg-zinc-900 border-l border-zinc-700 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2">
          ...
          {row("Raw bytes", (message.rawEnvelope?.length ?? 0) + " bytes", false)}
        </div>
      </div>
    );
  } catch (err) {
    return (
      <div className="w-80 bg-zinc-900 border-l border-zinc-700 flex flex-col shrink-0">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-700 text-red-400">
          <div className="flex items-center gap-2">
            <AlertCircle size={14} />
            <span className="font-semibold text-sm">Inspector Error</span>
          </div>
          <button onClick={onClose} className="text-zinc-500 hover:text-white"><X size={14} /></button>
        </div>
        <div className="p-4 text-xs text-zinc-400 space-y-2">
          <p>Failed to decode the VCO envelope for this message.</p>
          <div className="bg-zinc-950 p-2 rounded border border-red-900/30 font-mono break-words text-red-500/80">
