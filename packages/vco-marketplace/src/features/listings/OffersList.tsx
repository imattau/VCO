  ...
  if (offers.length === 0) {
    return (
      <div className="py-12 flex flex-col items-center justify-center bg-zinc-900/20 border border-dashed border-zinc-800 rounded-2xl">
        <p className="text-zinc-500 text-sm">No offers received yet.</p>
      </div>
    );
  }
          ...
          <div key={o.id} className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 flex items-center gap-4 hover:border-zinc-700 transition-all">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-sm font-bold text-white">{o.authorName}</span>
                <span className="text-[10px] text-zinc-500 uppercase font-black">Offer</span>
              </div>
              <div className="text-xs text-zinc-400 truncate">
                For: <span className="text-zinc-200 font-medium">{listing?.title ?? "Unknown Item"}</span>
              </div>
              {o.message && <div className="text-[11px] text-zinc-500 mt-1 italic italic">"{o.message}"</div>}
            </div>
